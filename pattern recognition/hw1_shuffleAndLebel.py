...
def random_take_data(mother_folder, train_num):
    ...
    for kid_folder in os.listdir(mother_folder):
        image_files = os.listdir(mother_folder + kid_folder)
        train_set = np.ndarray(shape=(train_num, image_size, image_size), dtype=np.float32)
        test_set = np.ndarray(shape=(len(image_files) - train_num, image_size, image_size), dtype=np.float32)
        image_files_train = np.random.choice(image_files, train_num, replace=False)
        image_files_test = np.setdiff1d(image_files, image_files_train)
