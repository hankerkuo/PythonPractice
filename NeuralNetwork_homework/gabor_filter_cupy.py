...
def gabor(lst, ksize, sigma, theta, lambd, gamma, threshold=0):
    ...
    result = cp.ndarray(shape=(np.shape(lst)[0], np.shape(lst)[1]))
    pad_size = (ksize - 1) // 2
    lst = cp.pad(lst, ((pad_size, pad_size), (pad_size, pad_size)), 'constant', constant_values=((0, 0), (0, 0)))
    for y in range(np.shape(result)[0]):
        for x in range(np.shape(result)[1]):
            result[y][x] = cp.sum([ele[x:x + ksize] for ele in lst[y:y + ksize]] * kernel)
