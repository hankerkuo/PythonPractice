...
CNN_layer2_weight = (np.random.rand(layer2_ch, 4, 4, 5, 5) - 0.5) * 2.5
    ...
    for index_y in range(4):
        for index_x in range(4):
            ...
            output_mat = np.array([ [ [ele[in_x - 2: in_x + 3] for ele in layer2_input[l1_ch][in_y - 2: in_y + 3]]
                                    ...
                                    * CNN_layer2_weight[l2_ch][index_y][index_x] for l2_ch in range(layer2_ch)])
        ...
        for index_x in range(4):
            ...
            for y in range(5):
                for x in range(5):
                    ...
                    for ch in range(layer2_ch):
                        CNN_layer2_weight[ch][index_y][index_x][y][x] = CNN_layer2_weight[ch][index_y][index_x][y][x] + \
                ...
                for y in range(-2, 3):
                    for x in range(-2, 3):
                        if 0 <= (in_ind_y + y) <= 7 and 0 <= (in_ind_x + x) <= 7:
                            single[8 * (in_ind_y + y) + (in_ind_x + x)] = CNN_layer2_weight[ch][index_y][index_x][y + 2][x + 2]
...
if __name__ == "__main__":
    ...
    np.save('Net5_Epochs_{} LRate_{} Filter_{} Time_{}.npy'
